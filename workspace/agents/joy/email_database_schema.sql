...
CREATE TABLE equipment (
    ...
    company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    equipment_id VARCHAR(100) NOT NULL,  -- custom admin-defined identifier
...
CREATE TABLE status_history (
    ...
    reason TEXT,  -- required for supervisor overrides
...
-- Example policy (repeat similar for all tables with company_id)
CREATE POLICY tenant_isolation ON users
    USING (company_id = current_setting('app.current_company_id', TRUE)::UUID);

-- Add similar policies for every table that has company_id:
-- CREATE POLICY tenant_isolation ON table_name USING (company_id = current_setting('app.current_company_id', TRUE)::UUID);
-- For INSERT/UPDATE/DELETE, you can use the same or more restrictive policies.

-- Force RLS (optional but recommended for stricter security)
-- ALTER TABLE users FORCE ROW LEVEL SECURITY;  -- etc.
...
BEGIN
    IF (TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status) THEN
        INSERT INTO status_history (
            ...
            OLD.status,
            NEW.status,
            current_setting('app.current_user_id', TRUE)::UUID,  -- set this in app session too
            COALESCE(current_setting('app.change_reason', TRUE), NULL),
            NULL  -- link to downtime_event if provided via session/app
        );
    END IF;
    RETURN NEW;
END;
...
-- Generic audit trigger function (for broad logging)
...
-- Example: attach to key tables (add more as needed; avoid high-volume tables to prevent bloat)
CREATE TRIGGER trg_audit_downtime_events
    AFTER INSERT OR UPDATE OR DELETE ON downtime_events
    FOR EACH ROW EXECUTE FUNCTION audit_log_changes();

-- =============================================
-- HELPER VIEWS FOR REPORTING (MTTR / MTBF)
-- =============================================

-- Example MTTR view (Mean Time To Repair)
...
FROM equipment e
JOIN downtime_events de ON de.equipment_id = e.id
LEFT JOIN work_logs wl ON wl.downtime_event_id = de.id
WHERE de.severity = 'critical'  -- or make configurable
GROUP BY e.company_id, e.id, e.name;

-- Example MTBF view (simplified; uses status history for uptime periods)
-- This is more complex; refine based on exact business rules for "operational time"
CREATE VIEW vw_mtbf AS
SELECT 
    company_id,
    entity_id,
    COUNT(*) AS failure_count,
    -- Placeholder: calculate total uptime minutes between failures using lag on status_history
    -- Full implementation often requires window functions or materialized views
    'TBD - implement with window functions on status_history' AS mtbf_note
FROM status_history
WHERE new_status = 'down'
GROUP BY company_id, entity_id;

-- Additional reporting views can be added here (downtime by zone, technician performance, etc.)

-- =============================================
-- FINAL NOTES / BEST PRACTICES
-- =============================================
-- 1. In your Node.js app: Set session variables on every connection (company_id, current_user_id, change_reason).
-- 2. Use transactions for status updates + downtime events to keep consistency.
-- 3. For roll-up logic: Implement in app or via a stored procedure that checks child statuses.
-- 4. Retention: Add a job to archive old downtime_events / audit_log (e.g., >2 years).
-- 5. Testing: Verify RLS with different company contexts; test triggers with concurrent updates.
-- 6. Extensions: Consider pg_cron for scheduled reports or TimescaleDB if high-volume time-series needed later.

-- Run this script in order. Verify with \d+ in psql after creation.
